legend_plot <- function(stat.type){
	...
	thick <- 4
	if(length(stat.type) == 0 | stat.type == 'none') {
		l <- nothing()
	} else if(stat.type %in% c('sens', 'spec', 'ppv', 'npv', 'pct', 'prev')) {
		l <- orange_over_sum(thick)
	} else { 
		l <- orange_over_green(thick)
	}
		
 	l <- l + coord_cartesian(xlim = c(-2, 2), ylim = c(-2, 1)) +
...
nothing <- function(){
	ret <- ggplot() + annotate('text', x = 0, y = .5, label = "No Selection")
...
orange_over_green <- function(thick){
